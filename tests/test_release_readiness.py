...
def test_release_version_is_v1_2():
    assert __version__ == "v1.3"
    assert config.APP_VERSION == "v1.3"
    assert "v1.3" in config.WINDOW_TITLE
...
def test_version_json_matches_release(tmp_path):
    ...
    assert version_data["version"] == "v1.3"
    assert version_data["download_url"].endswith("/releases/tag/v1.3")
