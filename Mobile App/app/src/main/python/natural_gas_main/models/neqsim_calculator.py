...
try:
    import os
    # Detect Chaquopy environment by trying to import the java module
    try:
        from java import jclass
        class NeqSimJavaWrapper:
            def __init__(self, prefix="org.neqsim"):
                self._prefix = prefix
            def __getattr__(self, name):
                path = f"{self._prefix}.{name}"
                # If name starts with capital letter, it is a Java Class name in NeqSim
                if name and name[0].isupper():
                    return jclass(path)
                return NeqSimJavaWrapper(path)
        
        _jneqsim = NeqSimJavaWrapper()
        NEQSIM_AVAILABLE = True
        logging.getLogger(__name__).info("NeqSim Android (Chaquopy) wrapper'ı başarıyla başlatıldı")
    except ImportError:
        # Fallback to desktop neqsim python package
        from neqsim import jneqsim as _jneqsim
        NEQSIM_AVAILABLE = True
        logging.getLogger(__name__).info("NeqSim Masaüstü başarıyla yüklendi")
